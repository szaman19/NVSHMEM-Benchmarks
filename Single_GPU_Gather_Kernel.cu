...
main(int argc, char * argv[]){
  ...
  constexpr int num_in_rows = 169343;
  const int num_out_rows = edges.num_edges();
  ...
  outfile.open("1_gather_arxiv.csv");
  ...
  for (size_t i = 0; i < count; i++){    
    ...
    const int* indices_vec = _to_device_ptr<int>(edges.get_target_indices(), num_out_rows);
    ...
    grid.x = 1;
    grid.y = (num_out_rows + block.y - 1) / block.y;
    grid.z = 1; 
    gather<<<grid, block>>>(value_matrix,
                            indices_vec,
                            output_matrix,
                            mini_batch_size,
                            num_in_rows,
  ...
  outfile<<'\n';
  std::cout << "Gather Elapsed Time (S)=" << average(timing_array) * 1E-9 << std::endl;
  outfile.close();
  return 0; 
}
